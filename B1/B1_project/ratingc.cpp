...
void getData_4();
void scoreSequence_4();   //冒泡法排序
void score2rating_4();    //将综评成绩转化为等级
void getPassrate_4();
extern float ave;
extern int stu_num;
float c_rate;
int c_num = 0;
student student_c[40];

void getPassrate_4(){
    for(int i = 0; i < stu_num; i++){
        if(students[i].overallScore >= 70 && students[i].overallScore < 80){
            student_c[c_num] = students[i];
            c_num++;
        }
    }
    c_rate = ((float)c_num / (float)stu_num) * 100;

}

// 冒泡法对所有学生成绩进行排序
void scoreSequence_4(){
    student temp;
...
ratingc::ratingc(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ratingc)
{
    ui->setupUi(this);
    ...
    ui->scoreTable->setRowCount(c_num);
    for(int i = 0; i < c_num; i++)
    {
        QString normalScore = QString("%1").arg(student_c[i].normalScore);
        QString halfScore = QString("%1").arg(student_c[i].halfexamScore);
        QString finalScore = QString("%1").arg(student_c[i].finalexamScore);
        QString overallScore = QString("%1").arg(student_c[i].overallScore);
        QString stuID = QString(QLatin1String(student_c[i].stu_num));
        QString rating = QString(student_c[i].Rating);
    ...
    QString cnum = QString("%1").arg(c_num);
    QString crate = QString("%1%").arg(c_rate);


    ui->c_number->setText(cnum);
    ui->c_rate->setText(crate);
}
