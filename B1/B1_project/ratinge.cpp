...
void getData_6();
void scoreSequence_6();   //冒泡法排序
void score2rating_6();    //将综评成绩转化为等级
void getPassrate_6();
extern float ave;
extern int stu_num;
float e_rate;
int e_num = 0;
student student_e[40];

void getPassrate_6(){
    for(int i = 0; i < stu_num; i++){
        if(students[i].overallScore < 60){
            student_e[e_num] = students[i];
            e_num++;
        }
    }
    e_rate = ((float)e_num / (float)stu_num) * 100;
}

// 冒泡法对所有学生成绩进行排序
void scoreSequence_6(){
    student temp;
...
ratinge::ratinge(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ratinge)
{
    ui->setupUi(this);
    ...
    ui->scoreTable->setRowCount(e_num);
    for(int i = 0; i < e_num; i++)
    {
        QString normalScore = QString("%1").arg(student_e[i].normalScore);
        QString halfScore = QString("%1").arg(student_e[i].halfexamScore);
        QString finalScore = QString("%1").arg(student_e[i].finalexamScore);
        QString overallScore = QString("%1").arg(student_e[i].overallScore);
        QString stuID = QString(QLatin1String(student_e[i].stu_num));
        QString rating = QString(student_e[i].Rating);
    ...
    QString dnum = QString("%1").arg(e_num);
    QString drate = QString("%1%").arg(e_rate);


    ui->e_number->setText(dnum);
    ui->e_rate->setText(drate);
}
